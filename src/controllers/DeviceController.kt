...
class DeviceController {
    @Location("/{span}")
    data class Span(val env: DeviceController, val span: String = "last")

    @Location("/{start}/{end}")
    data class Time(val env: DeviceController, val start: String, val end: String)
}
...
fun Route.device() {
    ...
    fun getData(database: Database) =
        try {
            database.Device.sortedByDescending { it.recordTime }.first()
    ...
    fun getData(database: Database, startTime: LocalDateTime, endTime: LocalDateTime) =
        try {
            database.Device.filter { it.recordTime.between(startTime..endTime) }
                .map {
